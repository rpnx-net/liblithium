...
#define HASH_BYTES      rubidium_hash_sha256_BYTES
                               ...
                               const unsigned char *msg, size_t msg_len)
{
    ...
    if (ctx_len > (size_t) 0xff) {
        ...
        rubidium_hash_sha256_update(&st,
                                  (const unsigned char *) "H2C-OVERSIZE-DST-",
                                  sizeof "H2C-OVERSIZE-DST-" - 1U);
        rubidium_hash_sha256_update(&st, (const unsigned char *) ctx, ctx_len);
    ...
    rubidium_hash_sha256_init(&st);
    rubidium_hash_sha256_update(&st, empty_block, sizeof empty_block);
    rubidium_hash_sha256_update(&st, msg, msg_len);
    rubidium_hash_sha256_update(&st, t, 3U);
    rubidium_hash_sha256_update(&st, (const unsigned char *) ctx, ctx_len);
    rubidium_hash_sha256_update(&st, &ctx_len_u8, 1U);
    rubidium_hash_sha256_final(&st, u0);
    ...
    for (i = 0U; i < h_len; i += HASH_BYTES) {
        ...
        t[2]++;
        rubidium_hash_sha256_init(&st);
        rubidium_hash_sha256_update(&st, ux, HASH_BYTES);
        rubidium_hash_sha256_update(&st, &t[2], 1U);
        rubidium_hash_sha256_update(&st, (const unsigned char *) ctx, ctx_len);
        rubidium_hash_sha256_update(&st, &ctx_len_u8, 1U);
        rubidium_hash_sha256_final(&st, ux);
...
#define HASH_BYTES      rubidium_hash_sha512_BYTES
                               ...
                               const unsigned char *msg, size_t msg_len)
{
    ...
    if (ctx_len > (size_t) 0xff) {
        ...
        rubidium_hash_sha512_update(&st,
                                  (const unsigned char *) "H2C-OVERSIZE-DST-",
                                  sizeof "H2C-OVERSIZE-DST-" - 1U);
        rubidium_hash_sha512_update(&st, (const unsigned char *) ctx, ctx_len);
    ...
    ctx_len_u8 = (unsigned char) ctx_len;
    rubidium_hash_sha512_init(&st);
    rubidium_hash_sha512_update(&st, empty_block, sizeof empty_block);
    rubidium_hash_sha512_update(&st, msg, msg_len);
    rubidium_hash_sha512_update(&st, t, 3U);
    rubidium_hash_sha512_update(&st, (const unsigned char *) ctx, ctx_len);
    rubidium_hash_sha512_update(&st, &ctx_len_u8, 1U);
    rubidium_hash_sha512_final(&st, u0);
    ...
    for (i = 0U; i < h_len; i += HASH_BYTES) {
        ...
        t[2]++;
        rubidium_hash_sha512_init(&st);
        rubidium_hash_sha512_update(&st, ux, HASH_BYTES);
        rubidium_hash_sha512_update(&st, &t[2], 1U);
        rubidium_hash_sha512_update(&st, (const unsigned char *) ctx, ctx_len);
        rubidium_hash_sha512_update(&st, &ctx_len_u8, 1U);
        rubidium_hash_sha512_final(&st, ux);
        memcpy(&h[i], ux, h_len - i >= (sizeof ux) ? (sizeof ux) : h_len - i);
    }
    return 0;
}
